...
- **Pin it open.** A pinned outline stays at the full height of the pane — in the editor and the viewer, in every window, across a restart — and by default the note text moves aside to make room for it, so it never covers a word. It stays as narrow as its headings allow, up to a maximum width you set. `Ctrl+Alt+P` pins and unpins from anywhere, and a setting does the same.
- **A toolbar on the outline, if you want one.** Switch it on and the outline grows a first row of controls: how wide it may grow (25, 33 or 50 % of the pane, or any width you type), how deep its headings go, and the pin. It is a convenience, not a gate — all three work from **Settings → Ridgeline** with the toolbar off.
...
- **Right-to-left headings read right-to-left.** An Arabic, Persian or Hebrew heading is aligned to the right, indented from the right and, when it is too long for the outline, trimmed at its end rather than its beginning. The direction is worked out for each heading on its own, so right-to-left and left-to-right headings mix freely in one note, and the minimap bars mirror in the same way.
